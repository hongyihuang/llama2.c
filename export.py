...
def quantize_q80(w, group_size):
    ...
    # temporary set every scale to one number, works for small models!
    # wmax = torch.ones(int(w.numel()/64))*torch.abs(w).max()
...
def quantize_int8(w):
    ...
    w = w.float() # convert to float32
    # find the max in each group
    wmax = torch.abs(w).max().item()
    # calculate the scaling factor such that float = quant * scale
    scale = wmax / 127.0
    # scale into range [-127, 127]
    quant = w / scale
    # round to nearest integer
    int8val = torch.round(quant).to(torch.int8).flatten()
    # dequantize by rescaling
    fp32val = int8val.float() * scale
...
def version3_export(model, filepath):
    """
    Export the model weights in Q8_0 into .bin file to be read from C. No group size.
    That is:
    - quantize all weights to symmetric int8, in range [-127, 127]
    - all other tensors (the rmsnorm params) are kept and exported in fp32
    - quantization is has *no* groups and transposed for q, k, v, w1, w2, & w3 for activation sparsity
    - tailored for small language models (<=3B) where there is ~25% activation sparsity and no outliers
    """
    version = 3
    ...
    weights = [
        model.tok_embeddings.weight,
        *[layer.attention.wq.weight.transpose(0, 1) for layer in model.layers],
        *[layer.attention.wk.weight.transpose(0, 1) for layer in model.layers],
        *[layer.attention.wv.weight.transpose(0, 1) for layer in model.layers],
        *[layer.attention.wo.weight.transpose(0, 1) for layer in model.layers],
        *[layer.feed_forward.w1.weight.transpose(0, 1) for layer in model.layers],
        *[layer.feed_forward.w2.weight.transpose(0, 1) for layer in model.layers],
        *[layer.feed_forward.w3.weight.transpose(0, 1) for layer in model.layers],
    ]
    shared_classifier = torch.equal(model.tok_embeddings.weight, model.output.weight)
    if not shared_classifier:
        weights.append(model.output.weight)
    print("Shared classifier", shared_classifier)
    ...
    for i, w in enumerate(weights):
        ...
        if (bins is None):
            ...
            bins += torch.bincount(q.flatten().long()+128, minlength=256)
        # save the int8 weights to file
        out_file.write(struct.pack('f', s)) # save scale factor
        serialize_int8(out_file, q) # save the tensor in int8
